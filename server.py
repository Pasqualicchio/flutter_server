...
@app.route('/upload_advanced_ui')
@login_required
def upload_advanced_ui():
    return render_template('uploadadvancedui.html')  # Modifica il nome del file qui
...
def upload():
    try:
        ...
        if not all([macro, commessa, matricola, item]) or not images:
        ...
        for image in images:
            ...
            image_path = os.path.join(macro_folder, filename)
